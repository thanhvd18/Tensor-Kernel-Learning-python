...
def sparse_graph( W,kNN=3):
  W_i = np.copy(W)
  N = W_i.shape[0]
  S = np.zeros_like(W_i)
  for row in range(N):
    indices = np.argsort(-W_i[row, :]) #decending order
    k_neighbors = indices[1:kNN+2]  # Skip the self-loop
...
def cross_diffusion(K_ten_, iterations,kNN=3):
